...
public class DuplicateRemoval {
	public static void main(String[] args) {
		...
		original = input.next(); //makes the original string whatever the user inputs
		...
		for(int i = 0; i < original.length() - 1; i++) { //beginning from index 0, will increment until the last character of the input
				if(originalArray[i] == originalArray[i+1]) { //checks to see if two consecutive characters are duplicates
